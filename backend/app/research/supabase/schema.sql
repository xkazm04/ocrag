...
CREATE TABLE IF NOT EXISTS research_findings (
    ...
    -- Sub-query tracking (NEW)
    sub_query_id TEXT,
    decomposition_context TEXT,
...
-- Expert persona analyses (topic-level)
-- ============================================
CREATE TABLE IF NOT EXISTS research_perspectives (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    session_id UUID REFERENCES research_sessions(id) ON DELETE CASCADE NOT NULL,

    -- Perspective type (extended for new agents)
    ...
    -- Specialized data for each perspective type (NEW)
    specialized_data JSONB,
    -- For historical: {parallels, patterns, consequences}
    -- For financial: {cui_bono, mechanisms, flows, sanctions}
    -- For journalist: {contradictions, propaganda, questions}
    -- For conspirator: {theory, probability, evidence}
    -- For network: {relationships, intermediaries, patterns}
...
CREATE INDEX IF NOT EXISTS idx_findings_subquery ON research_findings(sub_query_id);
...
-- For high-volume tables (findings, perspectives, sources)
-- ============================================
...
-- Findings: Session + type is the most common query pattern
CREATE INDEX IF NOT EXISTS idx_findings_session_type
    ON research_findings(session_id, finding_type);

-- Findings: Session + temporal context for timeline queries
CREATE INDEX IF NOT EXISTS idx_findings_session_temporal
    ON research_findings(session_id, temporal_context, event_date);

-- Findings: Covering index for session listing
CREATE INDEX IF NOT EXISTS idx_findings_session_cover
    ON research_findings(session_id, finding_type)
    INCLUDE (content, summary, event_date, confidence_score);

-- Perspectives: Session + type with covering columns
CREATE INDEX IF NOT EXISTS idx_perspectives_session_type_cover
    ON research_perspectives(session_id, perspective_type)
    INCLUDE (analysis_text, confidence);

-- Sources: Session + domain for source analysis
CREATE INDEX IF NOT EXISTS idx_sources_session_domain
    ON research_sources(session_id, domain, credibility_score DESC);

-- Sources: Covering index for listings
CREATE INDEX IF NOT EXISTS idx_sources_session_cover
    ON research_sources(session_id)
    INCLUDE (url, title, domain, credibility_score, source_type);

-- Claims: Topic + type for knowledge browsing
CREATE INDEX IF NOT EXISTS idx_claims_topic_type
    ON knowledge_claims(topic_id, claim_type)
    WHERE is_current = TRUE;

-- Claims: Verification status for review workflows
CREATE INDEX IF NOT EXISTS idx_claims_verification_topic
    ON knowledge_claims(verification_status, topic_id)
    WHERE is_current = TRUE;

-- Entities: Type + mention count for leaderboards
CREATE INDEX IF NOT EXISTS idx_entities_type_mentions
    ON knowledge_entities(entity_type, mention_count DESC);

-- ----------------------------------------
-- PARTIAL INDEXES for filtered queries
-- ----------------------------------------

-- Findings: Only promoted findings
CREATE INDEX IF NOT EXISTS idx_findings_promoted_only
    ON research_findings(session_id, knowledge_claim_id)
    WHERE is_promoted = TRUE;

-- Findings: Findings with dates (for timeline)
CREATE INDEX IF NOT EXISTS idx_findings_dated
    ON research_findings(session_id, event_date)
    WHERE event_date IS NOT NULL;

-- Claims: Only current claims (excludes superseded versions)
CREATE INDEX IF NOT EXISTS idx_claims_current_only
    ON knowledge_claims(topic_id, claim_type, created_at DESC)
    WHERE is_current = TRUE;

-- Claims: Disputed claims (need review)
CREATE INDEX IF NOT EXISTS idx_claims_disputed
    ON knowledge_claims(topic_id, updated_at DESC)
    WHERE verification_status = 'disputed';

-- Claims: High confidence claims
CREATE INDEX IF NOT EXISTS idx_claims_high_confidence
    ON knowledge_claims(topic_id, confidence_score DESC)
    WHERE is_current = TRUE AND confidence_score >= 0.8;

-- Sources: High credibility sources
CREATE INDEX IF NOT EXISTS idx_sources_credible
    ON research_sources(session_id, domain)
    WHERE credibility_score >= 0.7;

-- Entities: Verified entities only
...
-- Claims: Tag-based search
CREATE INDEX IF NOT EXISTS idx_claims_tags_gin
    ON knowledge_claims USING GIN(tags);

-- Entities: Alias search
CREATE INDEX IF NOT EXISTS idx_entities_aliases_gin
    ON knowledge_entities USING GIN(aliases);

-- Topics: Path hierarchy search
CREATE INDEX IF NOT EXISTS idx_topics_path_gin
    ON knowledge_topics USING GIN(path);

-- Sessions: Topic IDs search
CREATE INDEX IF NOT EXISTS idx_sessions_topics_gin
    ON research_sessions USING GIN(topic_ids);

-- Findings: Related findings search
CREATE INDEX IF NOT EXISTS idx_findings_related_gin
    ON research_findings USING GIN(related_findings);

-- Findings: Contradicts search
CREATE INDEX IF NOT EXISTS idx_findings_contradicts_gin
    ON research_findings USING GIN(contradicts);

-- Sources: Supporting sources search
CREATE INDEX IF NOT EXISTS idx_findings_sources_gin
    ON research_findings USING GIN(supporting_sources);

-- Perspectives: Specialized data search
CREATE INDEX IF NOT EXISTS idx_perspectives_data_gin
    ON research_perspectives USING GIN(specialized_data);

-- Findings: Extracted data search
CREATE INDEX IF NOT EXISTS idx_findings_data_gin
    ON research_findings USING GIN(extracted_data);
...
CREATE INDEX IF NOT EXISTS idx_findings_created_brin
    ON research_findings USING BRIN(created_at)
    WITH (pages_per_range = 32);

CREATE INDEX IF NOT EXISTS idx_perspectives_created_brin
    ON research_perspectives USING BRIN(created_at)
    WITH (pages_per_range = 32);

CREATE INDEX IF NOT EXISTS idx_sources_discovered_brin
    ON research_sources USING BRIN(discovered_at)
    WITH (pages_per_range = 32);

CREATE INDEX IF NOT EXISTS idx_claims_created_brin
    ON knowledge_claims USING BRIN(created_at)
    WITH (pages_per_range = 32);

CREATE INDEX IF NOT EXISTS idx_sessions_created_brin
    ON research_sessions USING BRIN(created_at)
    WITH (pages_per_range = 32);
...
CREATE INDEX IF NOT EXISTS idx_findings_session_hash
    ON research_findings USING HASH(session_id);

CREATE INDEX IF NOT EXISTS idx_sources_session_hash
    ON research_sources USING HASH(session_id);

CREATE INDEX IF NOT EXISTS idx_claims_topic_hash
    ON knowledge_claims USING HASH(topic_id);

CREATE INDEX IF NOT EXISTS idx_entities_type_hash
    ON knowledge_entities USING HASH(entity_type);
...
-- Uncomment if pg_trgm is enabled:
-- CREATE EXTENSION IF NOT EXISTS pg_trgm;
-- CREATE INDEX idx_claims_content_trgm ON knowledge_claims USING GIN(content gin_trgm_ops);
-- CREATE INDEX idx_findings_content_trgm ON research_findings USING GIN(content gin_trgm_ops);
...
ALTER TABLE research_findings ALTER COLUMN session_id SET STATISTICS 500;
ALTER TABLE research_findings ALTER COLUMN finding_type SET STATISTICS 200;
ALTER TABLE research_perspectives ALTER COLUMN session_id SET STATISTICS 500;
ALTER TABLE research_sources ALTER COLUMN session_id SET STATISTICS 500;
ALTER TABLE research_sources ALTER COLUMN domain SET STATISTICS 300;
ALTER TABLE knowledge_claims ALTER COLUMN topic_id SET STATISTICS 500;
ALTER TABLE knowledge_claims ALTER COLUMN claim_type SET STATISTICS 200;
ALTER TABLE knowledge_entities ALTER COLUMN entity_type SET STATISTICS 200;

-- ----------------------------------------
-- TABLE STORAGE OPTIMIZATION
-- ----------------------------------------

-- High-volume tables: Lower fillfactor for HOT updates
ALTER TABLE research_findings SET (fillfactor = 90);
ALTER TABLE research_sources SET (fillfactor = 90);
ALTER TABLE knowledge_claims SET (fillfactor = 85);

-- Aggressive autovacuum for high-volume tables
ALTER TABLE research_findings SET (
    autovacuum_vacuum_scale_factor = 0.05,
    autovacuum_analyze_scale_factor = 0.02
);

ALTER TABLE research_sources SET (
    autovacuum_vacuum_scale_factor = 0.05,
    autovacuum_analyze_scale_factor = 0.02
);

ALTER TABLE research_perspectives SET (
    autovacuum_vacuum_scale_factor = 0.05,
    autovacuum_analyze_scale_factor = 0.02
);

ALTER TABLE knowledge_claims SET (
    autovacuum_vacuum_scale_factor = 0.05,
    autovacuum_analyze_scale_factor = 0.02
);
